...
| **Cratered asteroid polygon** (radial jitter + 2–3 crater arcs `rgba(255,255,255,0.08)` + `rgba(0,0,0,0.35)`) | dmcinnes/HTML5-Asteroids `generatePolygon` + Rough.js jitter idea | Each rock unique, massive game-feel win | `render.js: asteroidPath()` |
...
- **Goal:** Unique cratered/rough rock readable at 960×600 and across `seamCopies(x,y,r)` wrap (up to 4 copies at `±W/±H`); silhouette must read as rock, not blob.
